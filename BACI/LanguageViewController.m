...
- (void) handleTimer: (id) atimer
{
	...
	if(iAnimation > [animationArray count]-1){
        ...
        UIImage *image3 = [UIImage imageNamed:@"splash.png"];
...
- (void)createLanguageBar{
	...
	if(adjustToolbar == 0){
		adjustToolbar = 1;
        toolbarLang = [[UIToolbar alloc] initWithFrame:CGRectMake(0, 768-64, screenWidth, 44)];
	}else{
        toolbarLang = [[UIToolbar alloc] initWithFrame:CGRectMake(0, 768-44, screenWidth, 44)];
	}
	...
	langLabel.titleLabel.textAlignment = NSTextAlignmentCenter;//UITextAlignmentCenter;
	...
	[toolbarLang setItems:newItems];
	[langLabel release];
	[langButton release];
	toolbarLang.translucent = NO;
    toolbarLang.barStyle = UIBarStyleBlack;
	[toolbarLang sizeToFit];
    [self.view addSubview:toolbarLang];
	[self.view bringSubviewToFront:toolbarLang];
...
- (void)initAnimationViews{
	...
	langView = [[LangView alloc] initWithFrame:boxframe];
	
	// Position the options at bottom of screen
	CGRect boxFrame = langView.frame;
	boxFrame.origin.x = 0;
	boxFrame.origin.y = screenHeight+boxFrame.size.height+200;
	langView.frame = boxFrame;
...
-(void)changeView:(id)sender{
	...
	CGRect optionsFrame = langView.frame;
    [UIView beginAnimations:nil context:nil];
	
	optionsFrame.origin.y = 768-(64+44);
	langView.hidden = NO;
    langView.frame = optionsFrame;
	langView.opaque = NO;
	langView.backgroundColor = [UIColor clearColor];
	
	[UIView commitAnimations];
	[self.view bringSubviewToFront:langView];
    
	[toolbarLang removeFromSuperview];
