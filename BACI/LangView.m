...
- (BOOL)webView:(UIWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType {	//linking the javascript to call the iPhone control
	...
	if(url != nil) {			
            ...
            NSError *error;
            NSString *langPath = [NSString stringWithFormat:@"%@/%@.plist", appDelegate.baseDirectory,language];
        	NSString *langPath2 = [NSString stringWithFormat:@"%@/LocalizedString.plist",appDelegate.baseDirectory];
            if([[NSFileManager defaultManager]copyItemAtPath:langPath toPath:langPath2 error:&error]==NO)
                NSLog(@"copy %@ error.",langPath);
